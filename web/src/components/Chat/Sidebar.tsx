...
export const Sidebar: React.FC<Props> = ({ loading }) => {
    return (
        <aside className="min-w-[250px] max-w-[400px] w-[35%] bg-white drop-shadow-md flex flex-col">
            ...
            <div className="main-content w-full flex-1 flex flex-col items-center pt-4 gap-3">
...
export const RoomsList: React.FC<Props> = ({ loading }) => {
    ...
    return (
        <>
            <h3 className="w-full  text-xl font-semibold special-text-gradiente flex px-4 gap-2">
                <svg
                    width="26"
                    height="26"
                    viewBox="0 0 39 36"
                ...
                <>
                    ...
                    <ul className="w-full px-3">
                        {disconnectedRooms.map((room) => (
                            <li
                                key={room._id}
                                className="w-full border-solid border-gray-200 border-b-[1px] first:border-t-[1px]"
                            >
                                <RoomItem {...room} />
                            </li>
                        ))}
...
const RoomItem: React.FC<InnerRoom & { strategy?: (id: string) => void }> = ({
    ...
    return (
        <div className="h-14 flex items-center gap-2">
            <Avatar src={"nada"} size="small" rounded={true} />
